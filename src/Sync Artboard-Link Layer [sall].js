...
export default function() {
    ...
    for (let i = 0, len = allLinkPngs.length; i < len; i++) {
        if (IdArray.findIndex(item => item === settings.layerSettingForKey(allLinkPngs[i], 'ArtboardId')) === -1) {
    ...
    IdArray.forEach(item => {
        ...
        if (ArtboardIndex !== -1) {
            ...
            allLinkPngs.filter(item2 => settings.layerSettingForKey(item2, 'ArtboardId') === item).forEach(item3 => {

...
export default function() {
    ...
    let SyncResult = 0
    let NullResult = 0
    let SameNameResult = 0
    let SelectedPage = doc.selectedPage

    if (doc.pages.findIndex(item => item.name === "Sync: 将新的同名画板复制到这里" || item.name === "Sync: 剩余画板不存在或存在多个同名匹配项" || item.name === "Sync: done!") !== -1) {

        let SyncPage = doc.pages.find(item => item.name === "Sync: 将新的同名画板复制到这里" || item.name === "Sync: 剩余画板不存在或存在多个同名匹配项" || item.name === "Sync: done!")
        ...
        if (SelectedPage.id !== SyncPage.id) {
            SyncPageArtboard.forEach(item => {
                ...
                let SameNameArtboardsLen = SelectedPage.layers.filter(item2 => item2.name === item.name).length
                ...
                */
                ...
                //main function start
                let ToSyncIndex = SelectedPage.layers.findIndex(item2 => item2.name === item.name)

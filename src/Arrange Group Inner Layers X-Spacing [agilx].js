        ...
        if (item.type === "Group" && item.layers.length > 1) {
            ...
            for (let i = 0, len = PositionArray.length; i < len; i++) {
                let MinPositionIndex = item.layers.findIndex(
                    (item2) => item2.frame.x === PositionArray[i]
                );
                item.layers[MinPositionIndex].frame.x =
                    item.layers[MinPositionIndex].frame.x + 0.00000000001;
            ...
            for (let i = 0, len = ArrangedIndex.length; i < len; i++) {
                ...
                MinPosition =
                    MinPosition +
                    Math.abs(Width * Math.cos(RotationPI)) +
                    Math.abs(Height * Math.sin(RotationPI)) +
                    SetSpacing;
            }

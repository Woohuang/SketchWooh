...
export default function() {
    ...
    let SyncRadiusResult = 0
    let SyncShadowsResult = 0
    ...
    if (CollectMasters.length === 0) {
        CollectMasters = sketch.find('SymbolMaster').filter(item => item.name.indexOf("lobal") !== -1 && item.name.indexOf("⚙️") !== -1)
    }

    //get global symbol masters' instance control layer
    sketch.find('SymbolMaster').filter(item => item.name.indexOf("lobal") !== -1 && item.name.indexOf("⚙️") !== -1).forEach(item => {
